...
class Model (object):
    ...
    def prepare_dt_series (self, input_series, label_series):
        ...
        self._X_train_and_val, self._X_test, self._y_train_and_val, self._y_test = \
            train_test_split (np.ones (input_series.shape [0], 1), y)
        self._X_train, self._X_val, self._y_train, self._y_val = \
            train_test_split (self._X_train_and_val, self._y_train_and_val)
        ...
        for series_name, series_data in input_series.items ():
            ...
            dt_x_train_and_val, dt_x_test, dt_y_train_and_val, dt_y_test = \
                train_test_split (fs, y, test_size=0.1, random_state=self._seed)

            dt_x_train, dt_x_val, dt_y_train, dt_y_val = \
                train_test_split (dt_x_train_and_val, dt_y_train_and_val, test_size=0.1, random_state=self._seed)

            dt_model = DecisionTreeRegressor (max_depth=MAX_TREE_DEPTH)
            dt_model.fit (dt_x_train, dt_y_train)
                
            self._X_train = np.concatenate ((self.X_train, dt_model.predict (dt_x_train)), axis=1)
            self._X_val = np.concatenate ((self._X_val, dt_model.predict (dt_x_val)), axis=1)
            self._X_test = np.concatenate ((self._X_test, dt_model.predict (dt_x_test)), axis=1)
    ...
    def fit_and_summarize_random_forest_model (self):
        self._model = RandomForestRegressor (n_estimators=500)
        y_preds = None
        self._model.fit (self._X_train, self._y_train)
...
def main ():
    ...
    for lbda in lambda_vals:
        ...
        rf.fit_and_summarize_ridge_model (lbda)
